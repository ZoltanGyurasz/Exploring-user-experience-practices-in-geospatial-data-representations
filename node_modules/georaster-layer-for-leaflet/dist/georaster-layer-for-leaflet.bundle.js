...
var GeoRasterLayer = L.GridLayer.extend({
  ...
  getRasters: function getRasters(options) {
    ...
    */
    ...
    var rasterCoordsForTileCoords = function rasterCoordsForTileCoords(h, w) {
      var xCenterInMapPixels = innerTileTopLeftPoint.x + (w + 0.5) * widthOfSampleInScreenPixels;
      var yCenterInMapPixels = innerTileTopLeftPoint.y + (h + 0.5) * heightOfSampleInScreenPixels;
  ...
  drawTile: function drawTile(_ref) {
    ...
    try {
      var _mapCRS$transformatio, _mapCRS$transformatio2;
      ...
      var xScaleSignInMapCRS = Math.sign((mapCRS === null || mapCRS === void 0 ? void 0 : (_mapCRS$transformatio = mapCRS.transformation) === null || _mapCRS$transformatio === void 0 ? void 0 : _mapCRS$transformatio._a) || 1);
      var yScaleSignInMapCRS = Math.sign((mapCRS === null || mapCRS === void 0 ? void 0 : (_mapCRS$transformatio2 = mapCRS.transformation) === null || _mapCRS$transformatio2 === void 0 ? void 0 : _mapCRS$transformatio2._c) || -1);
      if (debugLevel >= 3) log({
        xScaleSignInMapCRS: xScaleSignInMapCRS,
        yScaleSignInMapCRS: yScaleSignInMapCRS
      });
      var xScaleOfScreenPixelInMapCRS = xScaleSignInMapCRS * widthOfScreenPixelInMapCRS;
      var yScaleOfScreenPixelInMapCRS = yScaleSignInMapCRS * heightOfScreenPixelInMapCRS;
      if (debugLevel >= 3) log({
        xScaleOfScreenPixelInMapCRS: xScaleOfScreenPixelInMapCRS,
        yScaleOfScreenPixelInMapCRS: yScaleOfScreenPixelInMapCRS
      });
      var snapped = (0, _snapBbox.default)({
        ...
        padding: [1, 1],
        // add extra padding to cautiously handle floating point arithmetic
        scale: [xScaleOfScreenPixelInMapCRS, yScaleOfScreenPixelInMapCRS]
      });
      if (debugLevel >= 3) console.log("[georaster-layer-for-leaflet] extent of inner tile before snapping " + extentOfInnerTileInMapCRS.reproj(inSimpleCRS ? "simple" : 4326).bbox.toString()); // reset inner tile to the snapped version

      extentOfInnerTileInMapCRS = new _geoExtent.GeoExtent(snapped.bbox_in_coordinate_system, {
        srs: inSimpleCRS ? "simple" : code
      });
      if (debugLevel >= 3) console.log("[georaster-layer-for-leaflet] extent of inner tile after snapping " + extentOfInnerTileInMapCRS.reproj(inSimpleCRS ? "simple" : 4326).bbox.toString()); // we round here because sometimes there will be slight floating arithmetic issues
      ...
      var xMaxOfInnerTileInMapCRS = extentOfInnerTileInMapCRS.xmax - padding.right * widthOfScreenPixelInMapCRS;
      ...
      if (debugLevel >= 3) console.log("setting tile height to " + innerTileHeight + "px"); // calculate height and width of the Leaflet tile
      // in the number of pixels from the original raster

      var rasterPixelsAcross = 0;
      var rasterPixelsDown = 0;
      ...
      if (inSimpleCRS) {
        ...
        var extentOfInnerTileInRasterCRS = extentOfInnerTileInMapCRS.reproj(this.projection);
      ...
      if (debugLevel >= 4) log({
        rasterPixelsAcross: rasterPixelsAcross,
        rasterPixelsDown: rasterPixelsDown
      });
      var resolution = this.options.resolution;
      var percentHeight = innerTileHeight / this.tileHeight;
      var percentWidth = innerTileWidth / this.tileWidth;
      if (debugLevel >= 4) log({
        percentHeight: percentHeight,
        percentWidth: percentWidth
      });
      var maxNumberOfSamplesAcross = Math.ceil(percentWidth * resolution);
      var maxNumberOfSamplesDown = Math.ceil(percentHeight * resolution);
      if (debugLevel >= 4) console.log({
        maxNumberOfSamplesAcross: maxNumberOfSamplesAcross,
        maxNumberOfSamplesDown: maxNumberOfSamplesDown
      }); // limit repeat sampling of the same pixel but also reducing aliasing at high zoom

      var numberOfSamplesAcross = Math.min(maxNumberOfSamplesAcross, 3 * rasterPixelsAcross);
      if (debugLevel >= 4) console.log({
        resolution: resolution,
        rasterPixelsAcross: rasterPixelsAcross,
        numberOfSamplesAcross: numberOfSamplesAcross
      });
      var numberOfSamplesDown = Math.min(maxNumberOfSamplesDown, 3 * rasterPixelsDown); // set how large to display each sample in screen pixels
  ...
  updateColors: function updateColors(pixelValuesToColorFn) {
    ...
    var _ref3 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {
      debugLevel: -1
        ...
        debugLevel = _ref3$debugLevel === void 0 ? -1 : _ref3$debugLevel;
