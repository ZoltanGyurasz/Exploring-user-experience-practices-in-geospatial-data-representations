...
var TileBoundingBoxUtils = /** @class */ (function () {
    ...
    TileBoundingBoxUtils.webMercatorTileBox = function (webMercatorBoundingBox, zoom) {
        ...
        var minLonClip = Math.max(-TileBoundingBoxUtils.WEB_MERCATOR_HALF_WORLD_WIDTH, webMercatorBoundingBox.minLongitude);
        var maxLonClip = Math.min(TileBoundingBoxUtils.WEB_MERCATOR_HALF_WORLD_WIDTH, webMercatorBoundingBox.maxLongitude);
        var minLatClip = Math.max(-TileBoundingBoxUtils.WEB_MERCATOR_HALF_WORLD_WIDTH, webMercatorBoundingBox.minLatitude);
        var maxLatClip = Math.min(TileBoundingBoxUtils.WEB_MERCATOR_HALF_WORLD_WIDTH, webMercatorBoundingBox.maxLatitude);
    ...
    TileBoundingBoxUtils.getWebMercatorBoundingBoxFromXYZ = function (x, y, zoom, options) {
        ...
        var box = new boundingBox_1.BoundingBox(minLon, maxLon, minLat, maxLat);
        return box;
    };
    /**
    ...
    TileBoundingBoxUtils.zoomLevelOfTileSize = function (tileSize) {
        var tilesPerSide = (2 * TileBoundingBoxUtils.WEB_MERCATOR_HALF_WORLD_WIDTH) / tileSize;
    ...
    TileBoundingBoxUtils.WEB_MERCATOR_HALF_WORLD_WIDTH = proj4_1.default('EPSG:4326', 'EPSG:3857').forward([180, 0])[0];
    TileBoundingBoxUtils.webMercatorProjection = proj4_1.default('EPSG:3857');
